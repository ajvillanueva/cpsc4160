...
FrameFactory::~FrameFactory() {
  ...
  for (auto textures = multiTextures.begin(); textures != multiTextures.cend();) {
    multiTextures.erase(textures);
    textures++;
  }

  for (auto frames = multiFrames.begin(); frames != multiFrames.cend();) {
    multiFrames.erase(frames);
    frames++;
  }
}
